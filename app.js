...
/* ── DOM 參考 ─────────────────────────────────────────────── */
let queryView, resultView, loadingOverlay, scheduleTitle, scheduleTableContainer;
...
function showView(viewId) {
    [queryView, resultView].forEach(v => {
        if (v) {
...
/* ═══════════════════════════════════════════════════════════
    計數器 CounterAPI / LocalStorage 邏輯
═══════════════════════════════════════════════════════════ */
async function updateCounters() {
    ...
    // 1. 本地 LocalStorage 統計（備用與即時顯示）
    let totalCount = parseInt(localStorage.getItem('sys_total_views') || '0', 10) + 1;
    localStorage.setItem('sys_total_views', totalCount);

    let monthData = JSON.parse(localStorage.getItem('sys_month_views') || '{}');
    if (monthData.month !== currentMonthKey) {
        monthData = { month: currentMonthKey, count: 1 };
    } else {
        monthData.count = (monthData.count || 0) + 1;
    }
    localStorage.setItem('sys_month_views', JSON.stringify(monthData));

    if (monthEl) monthEl.textContent = monthData.count;
    if (totalEl) totalEl.textContent = totalCount;

    // 2. 嘗試呼叫 CounterAPI 進行全域線上統計
    try {
        const namespace = 'minxiong_jh_schedule';
        
        // 累計點擊數 +1 並取得結果
        fetch(`https://api.counterapi.dev/v1/${namespace}/total/up`)
            .then(res => res.json())
            .then(data => { if (data && data.count && totalEl) totalEl.textContent = data.count; })
            .catch(() => {});

        // 當月點擊數 +1 並取得結果
        fetch(`https://api.counterapi.dev/v1/${namespace}/month_${currentMonthKey}/up`)
            .then(res => res.json())
            .then(data => { if (data && data.count && monthEl) monthEl.textContent = data.count; })
            .catch(() => {});
    } catch (e) {
        console.log("CounterAPI not available, fallback to localStorage.");
    }
}
...
async function fetchAndParseCSV(semLabel) {
    ...
    try {
        ...
        if (loadingOverlay) loadingOverlay.classList.remove('show');
        console.error(err);
        alert(`載入失敗：${err.message}。請確認 CSV/JSON 檔案路徑與設定。`);
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (semSelect && semSelect.value) {
        defaultSem = semSelect.value;
    } else if (typeof CONFIG !== 'undefined' && CONFIG.SEMESTERS) {
        defaultSem = Object.keys(CONFIG.SEMESTERS)[0] || '';
    }

    if (defaultSem) {
        await fetchAndParseCSV(defaultSem);
    } else {
        if (loadingOverlay) loadingOverlay.classList.remove('show');
    }
